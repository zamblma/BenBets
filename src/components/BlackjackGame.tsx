...
import { Play, DollarSign, Zap, Sparkles, Swords } from 'lucide-react';
...
function CardView({ card, index }: { card: Card; index: number }) {
  if (card.hidden) {
    return (
      <motion.div
        ...
        className="w-16 h-24 md:w-20 md:h-28 rounded-xl bg-gradient-to-b from-indigo-800 to-indigo-950 border border-indigo-400/30 flex items-center justify-center shadow-lg"
      >
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, scale: 1, x: 0 }}
      transition={{ delay: index * 0.1 }}
      className="w-16 h-24 md:w-20 md:h-28 rounded-xl bg-gradient-to-b from-slate-50 to-slate-200 border border-slate-400/40 flex flex-col items-center justify-center shadow-lg relative"
    >
...
export default function BlackjackGame({ balance, onUpdateBalance, onAddBetHistory }: BlackjackGameProps) {
  ...
  const stand = useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (dv < 17) {
        ...
        // blackjack check (player has blackjack, dealer doesn't)
  ...
  return (
    <div className="bg-gradient-to-b from-[#0a0b12] to-[#06070d] border border-[#1b1e2e] rounded-2xl p-3 sm:p-5 space-y-3 sm:space-y-4 shadow-[0_10px_30px_rgba(0,0,0,0.4)]">
      ...
      <div className="bg-gradient-to-r from-[#0d0e16] to-[#0a0b12] border border-[#1b1e2e] rounded-xl p-3">
        <div className="flex items-center justify-between">
          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1">
            <Swords className="w-3 h-3 text-brand" /> Blackjack 21
          </span>
      ...
      {/* Game Table */}
      <div className="bg-gradient-to-b from-[#0a3d2a] to-[#062015] rounded-2xl p-4 md:p-6 border border-emerald-900/40 shadow-[inset_0_0_60px_rgba(0,0,0,0.4)]">
        {/* Dealer Hand */}
        <div className="mb-6">
        ...
        {/* Divider */}
        <div className="border-t border-emerald-800/30 my-4" />
        ...
        <AnimatePresence mode="wait">
          {result && (
            ...
            >
              {result}
              {payout > 0 && (
                <span className="block text-xs font-mono mt-1">+R$ {payout.toFixed(2)}</span>
              )}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {status === 'betting' ? (
        <div className="space-y-3">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            className="w-full bg-gradient-to-r from-brand to-emerald-500 text-slate-950 font-black py-3 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer shadow-[0_0_20px_rgba(0,255,135,0.2)]"
          >
        ...
        <div className="space-y-3">
          {status === 'playing' && (
            <div className="flex gap-2">
              <button onClick={hit}
                className="flex-1 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-black py-3 rounded-xl text-xs uppercase tracking-wider cursor-pointer transition-all hover:shadow-[0_0_15px_rgba(245,158,11,0.3)]">
                Pedir (Hit)
              </button>
              <button onClick={stand}
                className="flex-1 bg-gradient-to-r from-slate-600 to-slate-700 text-white font-black py-3 rounded-xl text-xs uppercase tracking-wider cursor-pointer transition-all hover:shadow-[0_0_15px_rgba(148,163,184,0.3)]">
                Parar (Stand)
              </button>
              <button onClick={double}
                disabled={!canDouble}
                className="flex-1 bg-gradient-to-r from-purple-600 to-violet-700 disabled:from-[#1a1c29] disabled:to-[#1a1c29] disabled:text-[#383d5a] text-white font-black py-3 rounded-xl text-xs uppercase tracking-wider cursor-pointer disabled:cursor-not-allowed transition-all">
          ...
          {(status === 'settled') && (
            <motion.button
              ...
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.99 }}
              className="w-full bg-gradient-to-r from-brand to-emerald-500 text-slate-950 font-black py-3 rounded-xl text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer"
            >
